...
public class PaintActivity extends AppCompatActivity {
    ...
    public boolean onOptionsItemSelected(MenuItem item)
    {
        ...
        if(id==R.id.action_DelBig) {
            paintView.DelBig();
        }
        if(id==R.id.action_Save){
            Toast toast=null;
            paintView.setDrawingCacheEnabled(true);
            String imgSaved= MediaStore.Images.Media.insertImage(getContentResolver(),paintView.getDrawingCache(), "picture_"+countSaved+".png","drawing");
            if(imgSaved!=null){
               toast=Toast.makeText(this,"Save Successful",Toast.LENGTH_SHORT);
               countSaved++;
            }
            else{
                toast=Toast.makeText(this,"Save Unsuccessful",Toast.LENGTH_SHORT);
            }
            toast.show();
